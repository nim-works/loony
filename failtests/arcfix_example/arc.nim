...
proc nimDecWeakRef(p: pointer) {.compilerRtl, inl.} =
  atomicdec head(p).rc, rcIncrement
...
proc nimIncRef(p: pointer) {.compilerRtl, inl.} =
  ...
  atomicinc head(p).rc, rcIncrement
  when traceCollector:
    cprintf("[INCREF] %p\n", head(p))
...
proc nimDecRefIsLast(p: pointer): bool {.compilerRtl, inl.} =
  if p != nil:
    ...
    if (atomicLoadN(cell.rc.addr, ATOMIC_RELAXED) and not rcMask) == 0:
      result = true
      when traceCollector:
        cprintf("[ABOUT TO DESTROY] %p\n", cell)
    else:
      atomicdec cell.rc, rcIncrement
